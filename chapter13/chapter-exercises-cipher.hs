-- [[file:chapter-exercises.org::ciphers][ciphers]]
module Cipher where
...
runCaesar :: IO()
runCaesar = do
  putStrLn "Input the word to encrypt: "
  word <- getLine
  putStrLn "Input shift value: "
  shifter :: Int <- fmap read getLine
  let encrypted = caesar shifter word
  print encrypted

runVignere :: IO()
runVignere = do
  putStrLn "Input the word to encrypt: "
  word <- fmap Message getLine
  putStrLn "Input Keyword to encrypt with: "
  keyword <- fmap Keyword getLine
  let encrypted = vignere keyword word
  print encrypted
-- ciphers ends here
